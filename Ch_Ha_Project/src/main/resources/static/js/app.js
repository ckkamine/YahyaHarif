...
function adminProfileCtrl($scope, $http, $rootScope, $cookieStore) {
	var role = $rootScope.user.role.toLowerCase();
	var title= angular.element(document.querySelector('title'));
	title.html($rootScope.user.firstName+' '+$rootScope.user.lastName);
	switch (role) {
	case 'manager':
...
function bapCtrl($scope, $http, $rootScope) {
	...
	$scope.verifierLesObjectifs= function() {
		$scope.verifierLesObjectifsF=true;
		$scope.listIsNull=false;
		if($scope.bapCurrent.objectifsSortantes.length==0){
			
			$scope.listIsNull=true;
			$scope.verifierLesObjectifsF=false;
		}
		for(i=0; $scope.bapCurrent.objectifsSortantes.length >i; i++){
			if($scope.bapCurrent.objectifsSortantes[i].valide==false){
				$scope.verifierLesObjectifsF=false;
				break;
			}
		}
...
function parametreCtrl($scope, $http, $location, $rootScope, $cookieStore) {
	...
	$scope.changeAdd = function() {
		...
		case true:
			$scope.add = false;
			title.html('AMBDA | Utilisateurs');
			break;
		case false:
			$scope.add = true;
			$scope.u = null;
			title.html('AMBDA | Ajouter Utilisateur');
			break;
		}
		$scope.update = false;
	};
